# Index Negocios

> ...

- Web Site Selection: Detach the viz - from the sites (manually) -  point it to  a json file.
- Create a selection infrastructure - show the design to people and collec statistics - to evaluate the design.
- Improve the specs
- Create the style files - for reproduction.
- Improve the production line - for Software.

## References

- https://www.designarena.ai/
- https://www.designprompts.dev/
- https://prompts.chat/
- https://righteous-guardian-68f.notion.site/Computer-User-Interface-22535b6ff16f424eac56d9bca1cb757c?source=copy_link
- https://github.com/f/awesome-chatgpt-prompts
- [A Design of an AI Enable Software Production Model](https://gleaming-slice-bf8.notion.site/A-Design-of-an-AI-Enable-Software-Production-Model-2c1e2010b23f8028a55cd3052cb19ead?source=copy_link)
